...
/* global WebImporter */
/**
 * Parser for carousel-image. Base: carousel.
 * Source: https://wknd.site/ca/en/adventures/bali-surf-camp.html (mini image carousel).
 * Structure: 2 columns. Row 1 = block name. Each subsequent row = one slide:
 *   [ image cell, text cell ]. These slides are image-only, so the text cell is empty
 *   but retained to keep every row at the same column count.
 */
export default function parse(element, { document }) {
  ...
  slides.forEach((slide) => {
    ...
    // Optional text content (title/description/CTA) — usually absent for image carousels.
  ...
  const block = WebImporter.Blocks.createBlock(document, { name: 'carousel-image', cells });
